//! 1.0.0 compatibility canister used by PocketIC cross-version tests.
//!
//! The fixture is pinned to the crates.io `ic-sqlite-vfs` 1.0.0 release so later
//! changes can prove that published 1.0 images still upgrade and import cleanly.

use candid::CandidType;
use ic_sqlite_vfs::db::migrate::Migration;
use ic_sqlite_vfs::db::value::{to_sql_ref, ToSql};
...
fn kv_count() -> Result<u64, String> {
    let count = Db::query(|connection| {
        connection.query_scalar::<i64>("SELECT COUNT(*) FROM kv", ic_sqlite_vfs::params![])
    })
    .map_err(error_text)?;
...
fn db_meta() -> Result<DbMeta, String> {
    ...
    let stats = ic_sqlite_vfs::sqlite_vfs::stable_blob::storage_stats()
...
fn values_lookup_sql(count: usize, column: &str) -> String {
